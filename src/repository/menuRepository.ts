...
class MenuRepository {
  ...
  async getRecommendedMeal(userId: string) {
    try {
      const query =
        "select t_menu_item.id as id ,t_meal_type.id as mealTypeId, t_menu_item.name as Name,t_meal_type.mealType as Mealtype ,t_menu_item.averageRating as Rating , t_menu_item.sentimentScore as SentimentScore  from t_menu_item INNER JOIN t_recommendation ON t_recommendation.itemId = t_menu_item.id INNER JOIN t_meal_type ON t_recommendation.mealTypeId = t_meal_type.id INNER JOIN t_user_preferences ON t_user_preferences.userId = ? where t_recommendation.dateOfRecommendation = current_date() ORDER BY CASE WHEN t_menu_item.dietType = t_user_preferences.diet_preference THEN 0 ELSE 1 END,CASE WHEN t_menu_item.cuisineType = t_user_preferences.cuisine_preference THEN 0 ELSE 1 END, CASE WHEN t_menu_item.spiceLevel = t_user_preferences.spice_level THEN 0 ELSE 1 END,CASE WHEN t_menu_item.isSweet = t_user_preferences.hasSweetTooth THEN 0 ELSE 1 END";
  ...
  async getRecommendedMealStatus() {
    try {
      const query =
        "select t_menu_item.id as ItemId , t_menu_item.name as ItemName, t_meal_type.mealType as Mealtype, t_recommendation.noOfVotes as Votes from t_recommendation INNER JOIN t_menu_item ON t_recommendation.itemId = t_menu_item.id INNER JOIN t_meal_type ON t_recommendation.mealTypeId = t_meal_type.id  where dateOfRecommendation = current_date();";
